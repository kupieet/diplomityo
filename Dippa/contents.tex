...
the researcher. The list includes only metrics that were described as
important in \cref{sec:importantStatements} or metrics that were described in
two or more cases.

Velocity and effort estimate metrics were the most described metrics
with 15,12 occurrences and also they were considered important. Defect count,
customer satisfaction and work in progress metrics occurred 8,6,6 times
...
Finding 5: Based on the metric categorization by \citetsrc{fenton1998software}
...
agile development. Does this mean that aforementioned processes are not
considered important, or does it mean that agile practitioners consider that
there is less leeway in aforementioned processes so they do not need to be
measured?
...
Finding 7: Based on the metric categorization by
...
that measuring resources is not important. One implication
is that in agile development it is thought that there is a capable team and
they can improve themselves. Also, in general, agile methods are used for more
simple projects where resource issues are simpler than in larger, more
complex, projects. Another possibility is that measuring resources, especially
people, can lead to dysfunctional behaviour.

Another observation is that documentation is not measured, like requirement
specification. Instead, the focus is on the actual product and features which
aligns with the agile principles of concentrating on the software.
...
Finding 8: Based on the metric comparison in \Cref{tab:comparisonOfMetrics},
it seems that industry is using most of the metrics suggested by agile
literature. Although, Kanban and LeanSD metrics were not used that much, but
that is likely due to low number of cases using Kanban or LeanSD. There were
also many metrics that were not suggested in agile literature. This implies
that industrial agile teams tend to add custom metrics on top of metrics
suggested by agile literature. 




\section{Syyt ja effectit}
...
``Tell me how you  measure me, and I will tell you how I will behave" said
\citetsrc{goldratt2006haystack}. It seems agile methods don't give any special
protection from dysfunctional use of metrics, even when using one of the core
metrics of Scrum. Even though there weren't a lot of evidence for this, one
case showed strong evidence and another hinted, and also it is presumable that
dysfunctional use of metrics would rarely be reported.

\citetsrc{1667571} also discuss their experiences of improper metrics that
waste resources and skew team behaviour in counter-productive ways.
...
Finding 3: Characteristics of important metrics are identified. This study
describes important metrics based on descriptions of importance and amount of
evidence. There are some characteristics of important metrics that can be
pointed out: such as ease of use and that the metric utilises existing tools.
Also, based on the effects of metric use, it seems that the ability to provoke
discussion is a characteristic for important metrics. Value Stream Maps and
number of bounce backs initiated root cause analysis meetings [S3, S17].
Moreover, metrics were analysed in reflection meeting where problem and
improvement were identified [S28]. Furthermore, technical debt board provided
visibility on technical debt issues and it helped to create discussion to
decrease technical debt [S4]. \citetsrc{1667571} also lists as one of their
agile metric heuristics that metrics should provide fuel for meaningful
conversation. \fixme{tähän voisi sanoa jotain customer satisfactionista}
