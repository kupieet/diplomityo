...
Scrum got its name from rugby, where the team groups up and plans its next
move. Similarly in software development, the scrum team has daily meetings
...
Extreme Programming (XP) got its name from a set of principles and practices
that were emphasized to the extreme. For example, if you think testing is a
...
Lean Software Development (LeanSD) contains many attributes from Scrum
and XP but it also utilises principles from Lean manufacturing, which Toyota
used to build their competitive edge \citetsrc{womack2007machine}.
\fixme{pitäiskö tähän avata ne princiipit?}
...
\begin{longtable}{p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}}
  ...
  metrics found from this study}
    \toprule
          &       & \multicolumn{5}{c}{Found from the study} \\
    \midrule
    Metrics suggested by the methods & Method & Scrum & XP    & Kanban & LeanSD & "Agile" \\
    Effort estimate & Scrum, XP, LeanSD & Story points [S7, S8], task effort
    [S8], effort estimate [S12] & Effort estimate [S9] &       &       & Effort estimate [S15, S15, S15, S15], effort estimate kits [S17], story estimate [S29] \\
    Effort left & Scrum, XP &       &       &       &       & Remaining task effort [S3] \\
    Total work remaining & Scrum &       &       &       &       &  \\
    Burndown (Velocity) & Scrum, XP, LeanSD & Velocity [S8, S10], operations's
    velocity [S8], Sprint burndown [S16], release burndown [S16], planned velocity [S16], average velocity [S23] & Velocity [S6], burndown [S5, S27, S28] & Pseudo velocity [S23] & Velocity [S13] & Velocity [S2] \\
    Cumulative flows & Scrum &       &       &       &       &  \\
    Written and passed unit tests & XP, LeanSD &       &  number of automated passing test steps [S5, S27, S28], \textbackslash{}\# of unit tests [S14] &       &       & Running tested features [S1] \\
    Actual development time & XP    &       &       &       &       &  \\
    Load factor & XP    &       &       &       &       &  \\
    Work in progress & Kanban & Rate of requirements per phase [S21] &       & work in progress [S23, S24] &       & Work in progress [S17], inventory of requirements
over time [S20], \textbackslash{}\# of requirements per phase [S22] \\
    Lead time & Kanban &       & Lead time [S19] & Lead time [S24] & Lead time [S18] & Lead time [S22] \\
    Due date performance & Kanban &       &       &       &       &  \\
    Throughput & Kanban &       &       &       &       & Throughput [S26] \\
    Issues and blocked work items & Kanban &       &       &       &       &  \\
    Flow effiency & Kanban &       &       &       &       &  \\
    Initial quality & Kanban, LeanSD & \textbackslash{}\# of open defects [S7], \textbackslash{}\# of defects found in system test [S7] & faults per iteration [S5], number of defects [S10], number of new and open defects [S27] &       &       & Defect count after testing [S1],  open defects[S3], \textbackslash{}\# of defects in backlog [S25] \\
    Failure load & Kanban &       &       &       &       & Number of bounce backs [S17] \\
    Cycle time & LeanSD &       &       & Cycle time [S23] &       & Cycle time [S17] \\
    Value Stream Maps(Work time, wait time) & LeanSD &       &       &       & Value Stream Maps (processing time, wait time) [S18] &  \\
    Amount of written and passed acceptance tests per iteration & LeanSD &       &       &       &       & Number of test cases [S1], test success rate [S3], test failure rate [S3],  \\
    Not suggested &       & technical debt board [S4], technical debt in effort [S4], build status [S4], defects deferred [S7], Net Promoter Score [S7], revenue per customer [S11], task's expected end date [S12],  completed web pages [S12], task done [S12], cost performance index [S16], schedule performance index [S16], variance in
handovers [S21], requirement's cost types [S21] &  check-ins per day [S5, S27, S28], build status [S14], test coverage [S14], test growth ratio [S14], violations
of static code analysis [S14], hange requests per requirement [S19], fault slips [S19], implemented vs wasted requirements [S19]. maintenance effort [S19] &       & Fix time of failed build [S13], story flow percentage [S13], percentage of stories prepared for sprint [S13], velocity of elaborating features [S13] & Business value delivered [S1], customer satisfaction[S1], critical defects sent by customer [S3], team effectiveness [S3], Common tempo time [S17], customer satisfaction (kano analysis) [S17], number of requests from customers [S20], defect trend indicator [S25], predicted\textbackslash{}\# of defects [S25], queue [S26], story percent complete [S29], progress as working code [S30] \\
    \bottomrule
    \end{longtable}%
    %\end{comment}
    \fixme{remaining work == velocity}
...
primary measure of progress - which was also seen in this study.
...
software development. This study classifies and describes the main metric
types and areas that are reported in empirical studies. This study provides
descriptions of how and why metrics are used to support agile software
development.\fixme{lisää important metrics ja metrics categories} This study
also analyzed how the presented metrics support the twelve principles of Agile
Manifesto \citepsrc{beck2001agile}.
