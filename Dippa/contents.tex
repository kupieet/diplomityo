...
recommended by \citetsrc{6092576}. Process started by going through all quotes within one
code and giving each quote a more descriptive code describing the quote in
high level. Then the descriptive codes were organized in groups based on their
similarity, see e.g., \Cref{fig:whyMapExample} These groups were then given
...
Furthermore, testing, implementation and whole development cycle were measured
mostly internally in the Processes class. Only two metrics are related to
measuring Resources class. \fixme{Vois häivyttää tota internal/external
sävyä, ja ehkä lisää tilalle jotain diibadaabaa}
...
dysfunctional use of metrics, for example developers causing broken build if
...
Important metrics are listed in \Cref{tab:importantMetrics} sorted by amount
of evidence and an importance factor. Amount of evidence is based on the
amount of cases describing the metric. Metrics that were only mentioned by
name without any reasons of use, effect of use, or importance were not taken
into account. Importance factor is an assessment of the metric's importance by
the researcher from 1 (low) to 3 (high). The list includes only metrics that
...
with 15 and 12 occurrences, and also, they were both considered important.
...
compared to existing knowledge, and further implications are explored.
...
driven projects. Also, agile methods are used for more simple projects where
...
\textbf{Research Question 1.2:} \emph{How do the metrics found in
 industrial agile software development compare to those suggested by agile
 literature?}\\
...
drive dysfunctional behaviour. It seems agile methods don't give any special
protection from dysfunctional use of metrics, even when using one of the core
metrics of Scrum, velocity [S6]. \begin{comment} Even though there weren't a
...
has also experiences on problems with metrics and people, thus he had written
...
issues with preparations to sprints. They started then measuring
...
question why metrics are used in industrial agile development. If we then
compare how these found reasons compare to reasons mentioned by others.
...
are measured accurately by most successful software companies. Also, they plan
and estimate software projects accurately. In this study, there were almost no
productivity metrics, but quality was measured with defect counts and customer
satisfaction metrics. Furthermore, there was a lot of emphasis on planning and
estimation of software projects. What does the lack of productivity
...
to be more focused on providing value for their customers so they don't focus
...
mapped to found effects of metric use (\Cref{tab:howCategories}) found from
...
important metrics based on descriptions of importance and amount of evidence,
see \cref{sec:listOfImportantMetrics}. There are some characteristics of
...
analysis meetings [S3, S17]. Moreover, metrics were analysed in reflection
meeting where problem and improvement were identified [S28]. Furthermore,
...
metric for business value a team should define with business unit, proposed by
\citetsrc{1667571}, was not seen in this study. Furthermore,
...
software development is provided by numbers.
