...
code was tested [S14]. However, in Brown-field (legacy) projects it ss better
...
broken build is used as a KPI (Key Progress Indicator).
...
importance by the researcher from 1 (low) to 3 (high). The list includes only metrics that
were described as important in \cref{sec:importantStatements} or metrics that
were described in two or more cases.
...
Finally, the limitations of the study discussed.
...
the need for a mix of higher skilled people in agile projects compared to plan
driven projects. Also, agile methods are used for simpler projects where
