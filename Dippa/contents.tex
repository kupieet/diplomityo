...
a Slovenian publishing company, release burndown made the correlation clear
...
understand if project was in target for delivery. At Microsoft Corporation, they monitored
...
used to see whether the team could meet their goals, and if not, what could be
done [S5]. Burndown was also used to mitigate the risk where developers spend too
...
a developer has been able to complete a story compared with the estimate.
...
\end{comment}
In agile development, measurement is focused on the product and the process,
but not the people. Based on the metric categorization by
\citetsrc{fenton1998software} in \Cref{tab:metricsCategorization}, measurement
...
development assumes a capable team, and that they can improve themselves.
\citetsrc{boehm2003using} acknowledge that agile projects require people with
higher methodological skills than plan-driven projects. Also, agile methods
are used for simpler projects where resource issues are simpler than in
larger, more complex projects. Another possibility is that measuring
resources, especially people, can lead to dysfunctional behaviour. Another
observation is that documentation is not measured. Instead, the focus is on
the actual product and features which aligns with the agile principles,
...
processes are not considered important in agile development, and another
possibility is that they are so tied within 'implementation' so that
...
Industrial agile teams use metrics suggested by agile literature, but they
also use custom metrics. Based on the metric comparison in
\Cref{tab:comparisonOfMetrics}, it seems that industry is using most of the
metrics suggested by agile literature. Although, Kanban and LeanSD metrics
were not used that much, but that is likely due to low number of cases using
Kanban or LeanSD. There were also many metrics that were not suggested in
agile literature. This implies that industrial agile teams tend to add custom
metrics on top of the metrics suggested by agile literature.
...
Use of metrics can have negative effects. Based on the results in
\cref{sec:motivatePeople}, metrics can be used to cause negative effects and
drive dysfunctional behaviour. It seems agile methods do not give any special
protection from the dysfunctional use of metrics, even when using one of the
core metrics of Scrum, velocity [S6]. \begin{comment} Even though there weren't a
lot of evidence for this, one case showed strong evidence and another hinted
at negative effects of metric use, and also it is presumable that
dysfunctional use of metrics would rarely be reported. \fixme{perustelu
edelliseen?}
\end{comment}
 \citetsrc{1667571} also discuss
their experiences of improper metrics that waste resources and skew team
behaviour in counter-productive ways. Similarly, \citetsrc{grady1992practical}
has experiences on problems with metrics and people, thus he had written
a software metrics etiquette, for example advising against measuring
...
Industrial agile teams use situative metrics. This study describes the reasons
and effects of metric use. There is evidence on the use of situative metrics.
Situative metrics are created and used based on a need, for example as a solution to a problem. At
Systematic, they had issues with build's long fix times. They started
measuring fix time of broken build and showed the time next to the coffee
machine. It provoked discussion on the reasons for long fix times, and
eventually, the developers fixed the builds faster [S13]. Similarly, they had
issues with preparations for sprints. They started measuring
\emph{percentage of stories prepared for sprint} supported with a checklist.
Furthermore, they set an organizational objective of at least 100\% for that
metric. At Petrobras, they had problems with customers related to rework and
delays. They started measuring technical debt with a technical debt board that
...
\citetsrc{1667571} differentiate between organizational long-term 'metrics'
from short-term context driven 'diagnostics'. %Both types of metrics were also
% seen in this study. 
'Diagnostics' seem to be the same as the situative metrics described in this
study. These short term context driven 'diagnostics' or 'situative metrics'
could be something that is more unique to agile development. 
...
are measured accurately by the most successful software companies. Also, they
plan and estimate software projects accurately. In this study, there were almost no
productivity metrics, but quality was measured with defect counts and customer
satisfaction metrics. Furthermore, there was a lot of emphasis on the planning
and estimation of software projects. What does the lack of productivity
measurement mean? Or is the productivity measured with some other method than
traditional metrics, for example frequent deliveries to customer? Do agile
teams think that measuring productivity is not important?
...
%\emph{``Alignment of software development to business objectives''}
...
with customer based metrics instead of the traditional quality models, such as
ISO/IEC 25010 \citepsrc{10951538}.
...
mapped to found effects of metric use (\Cref{tab:howCategories}) found in
...
Characteristics of important metrics are identified. This study describes
important metrics based on descriptions of importance and the amount of
evidence, see \cref{sec:listOfImportantMetrics}. There are some characteristics of
important metrics that can be pointed out: ease of use and that the metric
utilises existing tools. Also, based on the effects of metric use, it seems
that the ability to provoke discussion is a characteristic for important
metrics. Value Stream Maps and number of bounce backs initiated root cause
analysis meetings [S3, S17]. Moreover, metrics were analysed in a reflection
meeting where a problem and an improvement were identified [S28]. Furthermore,
technical debt board provided visibility on technical debt issues and it
helped to create discussion to decrease technical debt [S4].
...
%interested in. 
Based on the found important metrics, it could be argued that industrial agile
teams value planning (effort estimate), progress tracking (velocity), pre-release quality (defect count) and
post-release quality (customer satisfaction). 
...
for the lack of these metrics in this study could be the focus of this study
to team metrics instead of metrics of the whole organization. 
...
First, the applicable quality metrics for
agile development and the relationship between pre-release quality metrics and
post-release quality are important directions of future research. Second, this
study found that planning and tracking metrics for iteration were often used
indicating a need to focus future research efforts on these areas. Third, use
of metrics for motivating and enforcing process improvements can be an
interesting future research topic. Fourth, the use of situative metrics and
the use of customer satisfaction metrics in industrial agile context should
be further investigated. Finally, the dysfunctional use of metrics and the
negative effects of metric provide an interesting future research topic. 

Metrics are used to improve but they can also have negative effects. One needs
to have a holistic understanding of metrics in order to successfully apply
them.
