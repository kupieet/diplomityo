...
\begin{itemize}
  \item Research Question 1: \emph{Which metrics are used?}
  \item Research Question 2: \emph{Why are metrics used?} 
  \item Research Question 3: \emph{What are the effects of using metrics?} 
  \item Research Question 4: \emph{Which metrics are important?}
\end{itemize} 
...
studies by context factors. \fixme{siisti}
...
Telecom was
the most represented domain with 10 cases, enterprise information systems was the
second with 7 cases and web applications with 4 cases. There were 15 other
domains or cases without domain information.
...
in the case. If it was not clear what method was used or if there were
multiple methods mentioned 'NA' was set as agile method.
...
\begin{longtable}{p{3cm}p{1,4cm}p{1,5cm}p{1cm}p{1,5cm}p{1,5cm}p{1,5cm}}
 ...
 \toprule
    Metrics suggested by the methods & Method & Scrum & XP    & Kanban & LeanSD & NA \\
    \midrule
...
sources. Some of the descriptions for the reasons of metric use might seem
to be incomplete. This is because the effects of metrics use are described in
the following \cref{sec:effects}. Also, some reason categories might seem to be
more effect than reason, for example \nameref{sec:balanceWorkflow} in
\cref{sec:balanceWorkflow}. However, the reasons for metric use are described
here for \nameref{sec:balanceWorkflow} and then the actual actions and effects
are described in the next \cref{sec:effects}.
...
technical debt, e.g by implementing automatic deployment systems and improving
source code unit testability.
...
complete metric giva normalized measure of progress compared to developer
...
Based on the results for important metrics (\cref{sec:importantMetrics}), some
characteristics of important metrics can be highlighted. It seems industrial
agile teams appreciated metrics that were easy to use, highly visual and they
had the ability to facilitate discussion on problems and possible
improvements. Also, it seems that customer satisfaction metrics are important
for the success of a project. Furthermore, effort estimation and velocity
metrics were described as important, as well as they were identified from many
primary studies.

\fixme{mittareiden vertailusta agile kirjallisuuteen pitäisi sanoa jotain,
mitä?}
\fixme{saisko nyt avatuista mittarikategorioista jotain ajatuksia tänne?}
