...
have been published.
Yet, the literature reviews have been written from an academic viewpoin. For example, Catal
...
\begin{itemize}
  ...
  \item RQ2: What actions do the use of metrics trigger? \fixme{tää
  peilaamaan effectejä}
...
\fixme{vois ehkä poistaa tän Backgroundin kokonaan. sit nää agile methods
läpät vaan osaks introa.}
...
many attributes from Scrum and XP but it also utilises principles from Lean manufacturing, which Toyota
used to build their competitive edge \citetsrc{womack2007machine}.
\fixme{pitäiskö tähän avata ne princiipit? No jotain pitäis keksiä.}

The methods above are often well defined and they state that you must follow
all of their practices or then there could be negative effects. Kanban
\citepsrc{anderson2010kanban}on the other hand is more of an evolutionary
process model that allows each Kanban implementation to be different, suited for each context. Some
principles however are the same in Kanban. For example Kanban system's are
always pull-systems. Work is pulled only whenever there is capacity compared
to some other systems where work is pushed to development according to demand.
The pull system is enabled by limiting work in progress (WIP). For example, if
wip limit is 2 for development and there is already two work items under
development then no more work can be started before either of them is
completed. 
...
\fixme{pitäiskö tää siirtää Methodin alle?}
...
\fixme{Tätä voisi myös rohkeesti laajentaa lisää.}
...
study is more about trying to understand a problem instead of trying to find a solution to
...
\begin{itemize}
  ...
  \item Don't collect metrics that are primarily used to compare teams. (There were cases where a researcher or management uses a metric to compare
...
lean sotku -casper. Enää vissiin kysymys et pitäiskö muutama lause kirjoittaa
agile methodista} Singlecase was most used research method with 18 studies,
then experience report 8 studies, multicase with 3 studies and finally survey with 2 studies. Telecom was the most represented domain with 10 cases,
...
in the case. If it wasn't clear what method was used or if there were multiple
methods mentioned 'NA' was set as agile method.
...
insight. For example in some cases experience reports [S4] provided more
valuable data than a high scoring research papers [S25].
...
\fixme{Tähän jotain diipadaapaa pehmentämään taulukoita, ehkä voisi kertoa
vielä tarkemmin miten taulukot on rakennettu ja mitä niissä sanotaa sokeille.}
...
According to the categorization, metrics are focused to externally measure
the product while features are measured internally. Measurement in process
...
\toprule
    Metrics suggested by the methods & Method & Scrum & XP    & Kanban & LeanSD & "Agile" \\
    \midrule
    Effort estimate & Scrum, XP, LeanSD & 2[S7], 1[S8], 2[S8], 3[S12] & 1[S9]
    &       &       & 1[S15], 2[S15], 3[S15], 4[S15], 6[S17], 1[S29] \\
    Velocity\footnote{Includes total work remaining from Scrum and effort left
    from Scrum and XP.}  & Scrum, XP, LeanSD & 3[S8], 2[S10],  4[S8],
    1[S16], 2[S16], 5[S16], 1[S23] & 1[S6], 1[S5], 1[S27], 1[S28] & 4[S23] & 5[S13] & 1[S2], 5[S3] \\
    ...
    Value Stream Maps (Work time, wait time) & LeanSD &       &       &      
    & 2[S18], 3[S18] &  \\
...
Metrics found from the primary studies are compared to the ones suggested by
agile literature in \Cref{tab:comparisonOfMetrics}. The metrics found from
literature are from Scrum \citepsrc{schwaber2011scrum}, XP \citepsrc{beck2004extreme}, Kanban
\citepsrc{anderson2010kanban} and LeanSD \citepsrc{poppendieck2003lean}. The
four rightmost columns are describing the agile method used in the primary
studies. In some primary studies it was hard to identify a specific
agile method, thus 'Agile' is used to describe those cases. The number before
...
described in primary studies. Many metrics found from the studies were not
suggested in agile literature.
...
\fixme{kuva viite tarina}
\fixme{Lisää myös pari muuta mielenkiintoista kuvaa tänne viitteineen.}
%Similarly, monitoring schedule and costs with a dashboard allowed to spot for
% improvement opportunities at OCLC [S31].

\section{What effects does the use of metrics have?}
\label{sec:effects}
...
considered important if the author, or case employees praised the metric.
Also, metrics were considered important if there were signs of continuous use
of the metric. Furthermore, if metrics had positive correlation to project
success in surveys, they were considered important.
...
implementing features. Also, they said that using both metrics \emph{``drive
behaviors to let teams go twice as fast as they could before''}. 

Story percent complete metric was considered valuable since it embraces test
driven development - no progress is made before test is written [S29]. Also,
...
complete metric gives normalized measure of progress compared to developer
...
story percent complete metric. Practitioners at Ericsson valued the
...
terms of prioritization [S9]. According to a survey, top performing teams at
Adobe Systems estimated backlog items with relative effort estimates [S7].
Similarly, pseudo-velocity was considered essential for release planning
[S23]. Moreover, burndown was valuable in meeting sprint commitments [S7].
Furthermore, managers said burndown was important in making decisions and
managing multiple teams [S5]. However, developers didn't consider burndown
important [S5]. According to an agile survey [S1], project success had
significant positive relationship with the following metrics: team velocity, business
value delivered, running testing features, defect count after testing and
...
time, processing time and queue time. Similarly, technical debt board which
visualized the status of technical debt in categories was considered important
because it gave a high level understanding of the problems [S4]. Moreover, the
technical debt board was then used to plan actions to remove the technical
debt. Furthermore, it was proven to be useful in their context.

Net Promoter Score, which measure the probability of a customer recommending
the product to another potential customer, was said to be \emph{``one of the
purest measures of success''} [S7]. According to a survey [S1], projects that
were said to be definitely successful 77\% measured customer satisfaction often or always.
...
would succeed. Similarly, defects deferred was seen as a good predictor of
post-release quality because it correlated with issues found by the customers
[S7].
...
of agile software development \citetsrc{beck2001agile} categorized by Patel et
al. \citetsrc{1579312}. For each paragraph the naming by Patel et al. is used
and references to the agile principles is provided by numbers. 

Communication and Collaboration (4th and 6th agile principles
\citetsrc{beck2001agile}) was reflected by metrics
...
metrics focused on progress tracking (\cref{sec:projectProgress}) and timely
completion of project goals, see \cref{sec:accomplishProjectGoals}. However,
...
prioritization of features, see \cref{sec:planning} and
\cref{sec:planningActions}. %Additionally, different metrics helped keeping
...
features from that iteration [S6]. Also, adding people to project to reach a
certain date [S5, S17] doesn't seem that agile compared to removing tasks.
Adding people can have a negative impact to progress, considering the lack of
knowledge and training time required.
...
Regarding metrics that could be stated as important, effort estimate and
velocity metrics were most mentioned and they were also described as
important. 
\fixme{Important metrics:tähän jotain järkeä. Vois lisätä Customer
satisfaction, defect metrics.}

\fixme{mittareiden vertailusta agile kirjallisuuteen pitäisi sanoa jotain,
mitä?}

%Tässä voisin kuvata omia ajatuksia kokonaisuutena

There were some clearly reactive metrics that were used to provide immediate
improvement, for example fix the build faster. On the other hand there were
metrics that were aimed more towards long-term process improvement, for
example many of the cases from Ericsson. However, many of these cases seemed
to be very pilot-type, as in some metric based process improvement approach
was used. In many cases these metrics seemed heavy and not likely to be
applied after the pilot.

If an average team would need to be described it would be described as
follows. They would estimate effort for tasks and they would follow their
progress using velocity. They measure pre-release quality with number of
defects and they are interested in customer satisfaction with some metric.
Additionally, they would have some customized metrics that were needed to
solve some problems, for example they would measure the amount of time it
takes to fix a build.
...
on. Lue vielä kerran important metrics ilman kielenhuoltoperspektiiviä.}
...
context driven 'diagnostics'. Both type of metrics were also seen in this
study. However, key metric for business value a team should define with
...
threat to external validity. Seven out of 30 studies were from enterprise
...
primary studies. According to the researcher's knowledge there is no previous
...
The results indicate that the reasons for the use metrics is focused on the
following areas:
\nameref{sec:planning}, \nameref{sec:progressTracking},
\nameref{sec:quality}, and \nameref{sec:identifyProblems}. Similarly, the
effects of metric use is focused on the following areas:
