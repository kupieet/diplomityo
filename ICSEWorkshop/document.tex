...
(-historialähestyminen(en ehkä ite oo tästä hirveen innokas)
--lähti koodimittareista
--jossain vaiheessa kiinnostuttiin prosessimittareista
--agiilit menetelmät alkavat yleistyä, miten hoitaa mittaaminen siellä? Kun
prosessi muuttuu miten mittaus, perinteiset mittarit raskaita(?).
-- EBSE:n korostaminen 

-tore lähestyminen
-- tavalliselle talliaiselle intro sceneen
-- syyt miks me tehään tällainen tutkimus
-- mitä hyötyjä tällä tutkimuksella on)
...
There are a few mapping studies on software metrics(tähän vois lisätä kitch
whats 2010:ssä olevat muut mut en tiiä mitä lisäarvoa ne tois).
...
(Tähän ehkä kai tyyliin et vähän etitty et löytykö mitään laajaa tutkimusta
agile mittareista) (Tässä voisi myös perustella vielä SR:n valitsemista
menetelmäksi -Why SR was selected? This is more a “understanding of problem”
...
are only checked in certain intervals(ei oo muistissa viitettä tähän). In some
cases there are defined limits in metrics that trigger improvement actions(en
muista viitettä tähän).
...
code presented to the customer(viite, ja pitäis lisätä resultteihinkin ellei
jo ole siellä).
...
(toinen judu mitä täällä voisi olla niin vertailu perinteisiin tai
